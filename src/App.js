...
export default function App() {
  ...
  const [imGalleryPos, setImGalleryPos] = useState("sticky");
  ...
  const onScrollHandler = (event) => {
    ...
    if (imageIndex === 52 || scrollY >= 300) {
      setImGalleryPos("relative");
    } else {
      setImGalleryPos("sticky");
    }
  ...
  const showImage = () => {
    setImageIndex((prev) => {
      ...
      if (scrollDirection === "down" && !(prev >= 52)) {
        return prev + 1;
      }
      return prev;
    });
  ...
  return (
      <div className="App" onScroll={onScrollHandler}>
        <div id="image-container" style={{ position: imGalleryPos }}>
          {images.map((im) => (
